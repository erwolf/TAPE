...
function displayPlan(myPlan, summerShow){
	...
	planHeading.innerHTML = "ACADEMIC PLAN: " + myPlan.name;
	...
	for(var i=0; i<years.length; i++){
		...
		for(var j=0; j<sems.length; j++){
			...
			var currSem = myPlan.currSemester;
			var currYear = myPlan.currYear;
...
function addYear(){
	var newYear = parseInt(myPlan.years[myPlan.years.length-1].name)+1;
...
function deleteYear(){
	...
	var yearToDelete = myPlan.years[myPlan.years.length-1];
	
	if(checkEmpty(yearToDelete)){
		myPlan.years.pop();
		displayPlan(displaySummer);
	} else{
